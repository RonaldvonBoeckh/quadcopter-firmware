...
cmd_t cmd;			//global cmd var for tracking incoming cmds from app controller
...
void Drone::go() {
	...
	Drone::com.send_msg("Init done, con est, in standby mode");
	...
	while(1) {
		...
		switch(Drone::command) {
			case CALIBRATE:
				...
				Drone::com.send_msg("Starting calibration");
				Drone::calibrate();
				Drone::com.send_msg("Done calibration");
				break;
			case STREAM:
				Drone::drone_status = streaming;
				Drone::com.send_msg("Starting streaming");
				Drone::stream(); 
				Drone::com.send_msg("Done streaming");
				break;
			case START_FLYING:
				Drone::com.send_msg("Starting flying");
				Drone::drone_status = flying;
				Drone::fly();
				Drone::com.send_msg("Landed");
				break;
...
void Drone::initialize(){
	...
	//send debug mesg
	Drone::com.send_msg("Connection established");
}
