 ...
 */
void Sensors::attitude_estimation_step(float dt, bool useAccel){
	...
	if(!useAccel){
		roll_accel_estimation = 0;
		pitch_accel_estimation = 0;
	}
	attitude.roll = (attitude.roll + gyrox * dt) * gyroCoeff + roll_accel_estimation * accelCoeff;
	attitude.pitch = (attitude.pitch + gyroy * dt) * gyroCoeff + pitch_accel_estimation * accelCoeff;
	attitude.yaw = attitude.yaw + gyroz * dt;
